...
public class AdjustComments extends Thread{

	//private final String FILE_OUT = "CDSegundoHAREM_TEMPO_COMENT.xml";
	//private static final String FILE_OUT = "CDSegundoHAREM_TEMPO.v8.COMENT.xml";
	private static final String OUTPUT_TERMINATION = "_COMENT.xml";
	...
	public AdjustComments(String cd){
	...
	public static void main(String args[]){
		...
		if(args.length == 0)
			System.err.println("Utilizacao:\n" +
					"java AdjustComments <coleccao_original.xml>");
		else
		{
			try {
				System.setOut(new PrintStream(getOutputFileName(args[0])));
			} catch (FileNotFoundException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			new AdjustComments(args[0]);
		}
	}
	...
	private static String getOutputFileName(String inputFile)
	{
		if(inputFile.contains(".xml"))
			return inputFile.replace(".xml", OUTPUT_TERMINATION);
		else
			return inputFile + "OUTPUT_TERMINATION";
	}
}
