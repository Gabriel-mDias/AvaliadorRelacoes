...
#    cd $diravaliacao/programas/Av_HAREM_XML;
    ...
    if (( $2==$classico )); then
	ficheiro_c=$submissao.${pista}.${str_tipo_cenario[$1]}
    else
    ...
    if (( $2==$rerelem )); then
	ficheiro_e=$submissao.${pista}_${str_tipo_cenario_rerelem[$4]}.${str_tipo_cenario[$1]}
    fi;
    ...
    rm -f $dirpartic/*.avalida $dirpartic/*.veu $dirpartic/*.expandido $dirpartic/*.selec $dirpartic/*.normalizado $dirpartic/*.triplas $dirpartic/*.filtrado $dirpartic/*.avaliado
    echo "Fim avaliação"
}
